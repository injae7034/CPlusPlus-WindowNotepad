...
void UpArrowKeyAction::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	//4. 이전 줄로 이동하고, 이전 줄의 위치와 글자위치를 저장한다.
	Long currentRowIndex = this->notepadForm->note->Previous();
	Long currentLetterIndex = this->notepadForm->current->GetCurrent();
	//5. 이동하기전 줄의 위치와 이동 후 줄의 위치가 다르면(실질적으로 이동을 했으면)
	if (previousRowIndex != currentRowIndex)
	{
		//5.1 현재 줄을 이전으로 이동한 후의 줄로 변경한다.
		this->notepadForm->current = this->notepadForm->note->GetAt(currentRowIndex);
		//5.2 이동하기 전에 캐럿의 위치가 줄의 제일 처음이 아니고 이전으로 이동한 후 줄의 개수가 0이 아니면
		...
		{
			//5.2.1 이전으로 이동하기 전 이전 캐럿의 위치까지 줄의 텍스트 폭을 구한다.
			...
			//5.2.2 캐럿의 현재 위치를 처음으로 이동시킨다.(현재 캐럿의 위치가 어딘지 모르기때문에)
			...
			//5.2.3 첫번째 글자를 읽기 위해 캐럿을 한칸이동시킨다.
			Long i = this->notepadForm->current->Next();
			//5.2.4 현재 줄에서 캐럿의 위치(i)까지 텍스트의 폭을 구한다.
			...
			{
				//5.2.5.1 캐럿의 현재 위치를 다음 칸으로 이동시킨다.
				i = this->notepadForm->current->Next();
				//5.2.5.2 캐럿의 위치까지 텍스트 폭을 구한다.
				currentRowTextWidth = this->notepadForm->textExtent->
				GetTextWidth(this->notepadForm->current->GetPartOfContent(i).c_str());
			}
			//5.2.6 현재 캐럿의 위치까지 텍스트 폭에서 이전 줄의 텍스트 폭을 뺀다.
			...
			//5.2.7 차이가 0보다 크면(현재 줄의 텍스트를 다 읽지 못한 경우)
			//현재 줄의 캐럿 위치까지 텍스트가 이전 줄의 캐럿 위치까지 읽은 텍스트보다 큰 경우
			if (difference > 0)
			{
				//5.2.7.1 캐럿의 현재 위치 이전까지의 글자 폭을 구한다.
		...
		//5.3 캐럿의 이전 위치가 0 또는 캐럿의 현재 줄의 글자개수가 0개이면
		else
		{
			//5.3.1 현재 캐럿의 위치를 0으로 이동시킨다.
