...
void CtrlBackSpaceKeyActionCommand::Execute()
{
	...
	if (this->notepadForm->isSelecting == false && this->isSelectedTextsRemoved == false)
	{
		...
		//2.5 현재 줄의 위치가 0보다 크고, 현재 글자 위치가 0이고, 현재 줄이 진짜 줄이면
			...
			&& !dynamic_cast<DummyRow*>(currentRow))
		{
			...
			//2.5.2 현재 줄의 이전 줄을 구한다.
			Glyph* previousRow = this->notepadForm->note->GetAt(currentRowPos - 1);
			//2.5.3 현재 줄의 이전 줄의 마지막 글자 위치를 구한다.
			Long letterPos = previousRow->GetLength();
			//2.5.4 현재 줄을 이전 줄에 합친다.
			currentRow->Join(previousRow);
			//2.5.5 Note에서 현재 줄의 주소를 지운다.
			this->notepadForm->note->Remove(currentRowPos);
			//2.5.6 현재 줄이 지워졌기 때문에 현재 줄을 변경한다.
			currentRowPos = this->notepadForm->note->GetCurrent();
			this->notepadForm->current = this->notepadForm->note->GetAt(currentRowPos);
			//2.5.7 현재 줄의 글자 위치가 지금은 마지막이기 때문에 변경해준다.
			//이전 줄의 마지막 현재 줄의 처음 사이에 위치하도록 조정한다.
			currentLetterPos = this->notepadForm->current->Move(letterPos);
			//2.5.8 Command에 변경 사항이 있음을 표시한다.
			this->isDirty = true;
		}
		//2.6 그 이외에는
		else
		{
			//2.6.1 이동하기 전에 현재 줄의 위치를 구한다.
			Long previousRowIndex = this->notepadForm->note->GetCurrent();
			//2.6.2 이동하기 전에 현재 글자의 위치를 구한다.
			Long previousLetterIndex = this->notepadForm->current->GetCurrent();
			//2.6.3 자동개행이 진행 중이 아니면
			...
			{
				//2.6.3.1 노트에서 단어단위로 왼쪽으로 이동한다.
				currentLetterIndex = this->notepadForm->note->PreviousWord();
				//2.6.3.2 이동한 후에 줄의 위치를 구한다.
				currentRowIndex = this->notepadForm->note->GetCurrent();
				//2.6.3.3 메모장의 현재 줄을 이동한 후의 줄로 변경한다.
				this->notepadForm->current = this->notepadForm->note->GetAt(currentRowIndex);
			}
			//2.6.4 자동개행이 진행 중이면
			else
			{
				//2.6.4.1 노트에서 자동개행 진행 중인 상태에서 단어단위로 왼쪽으로 이동한다.
				this->notepadForm->note->
					PreviousWordOnRowAutoChange(currentRowIndex, currentLetterIndex);
				//2.6.4.2 이동한 후에 줄의 위치를 구한다.
				currentRowIndex = this->notepadForm->note->GetCurrent();
				//2.6.4.3 메모장의 현재 줄을 이동한 후의 줄로 변경한다.
				this->notepadForm->current = this->notepadForm->note->GetAt(currentRowIndex);
			}
			//2.6.5 왼쪽 방향으로 단어단위로 이동한 다음에 현재 줄의 위치를 구한다.
			currentRowIndex = this->notepadForm->note->GetCurrent();
			this->notepadForm->current = this->notepadForm->note->GetAt(currentRowIndex);
			...
			if (this->isRedone == false)
			{
				//2.6.7.1 DummyRow를 생성한다.
				this->glyph = new DummyRow();
				//2.6.7.3 이동하기 전의 줄의 위치와 이동한 후의 줄의 위치가 서로 같으면
				if (previousRowIndex == currentRowIndex)
				{
					//2.6.7.3.1 왼쪽 방향으로 단어단위로 이동한 뒤의 글자위치가 
					//이동하기 전의 글자 위치보다 작은동안 반복한다.
					while (currentLetterIndex < previousLetterIndex)
					{
						//2.6.7.3.1.1 글자를 지우기 전에 글자를 구한다.
						letter = this->notepadForm->current->GetAt(currentLetterIndex);
						//2.6.7.3.1.2 글자를 깊은 복사해서 DummyRow에 저장한다.
				...
				//2.6.7.4 이동하기 전의 줄의 위치와 이동한 후의 줄의 위치가 서로 다르면
				else if (previousRowIndex > currentRowIndex)
				{
					//2.6.7.4.1 이동하기 전의 줄을 구한다.
					nextRow = this->notepadForm->note->GetAt(previousRowIndex);
					//2.6.7.4.2 이동하기 전의 글자위치까지 반복한다.
					i = 0;
					while (i < previousLetterIndex)
					{
						//2.6.7.4.2.1 글자를 구한다.
						letter = nextRow->GetAt(i);
						//2.6.7.4.2.2 글자를 깊은 복사해서 DummyRow에 저장한다.
						this->glyph->Add(letter->Clone());
						//2.6.7.4.2.3 글자를 지운다.
						nextRow->Remove(i);
						previousLetterIndex--;
					}
					//2.6.7.4.3 나중에 현재 줄에 Join시키기 위해 줄의 주소를 저장한다.
					row = nextRow;
					//2.6.7.4.4 이동하기 전의 줄이 이동한 후의 줄의 위치보다 큰 동안 반복한다.
					previousRowIndex--;
					while (previousRowIndex > currentRowIndex)
					{
						//2.6.7.4.4.1 이동하기 전의 줄을 구한다.
						nextRow = this->notepadForm->note->GetAt(previousRowIndex);
						i = nextRow->GetLength() - 1;
						//2.6.7.4.4.2 이동하기 전의 줄의 개수만큼 반복한다.
						while (i >= 0)
						{
							//2.6.7.4.4.2.1 글자를 구한다.
							letter = nextRow->GetAt(i);
							//2.6.7.4.4.2.2 글자를 깊은 복사해서 DummyRow에 저장한다.
							this->glyph->Add(0, letter->Clone());
							//2.6.7.4.4.2.3 글자를 지운다.
							nextRow->Remove(i);
							i--;
						}
						//2.6.7.4.4.3 글자를 다지웠기 때문에 줄을 지운다.
						this->notepadForm->note->Remove(previousRowIndex);
						//2.6.7.4.4.4 이전 줄로 이동한다.
						previousRowIndex--;
					}
					//2.6.7.4.5 현재 줄을 구한다.
					currentRow = this->notepadForm->current;
					//2.6.7.4.6 이전 줄을 다 지웠기 때문에 현재 줄의 글자들을 지운다.
					//현재 줄의 글자 개수보다 현재 글자의 위치가 크거나 같은 동안 반복한다.
					letterCount = currentRow->GetLength() - 1;
					while (letterCount >= currentLetterIndex)
					{
						//2.6.7.4.6.1 글자를 지우기 전에 글자를 구한다.
						letter = currentRow->GetAt(letterCount);
						//2.6.7.4.6.2 글자를 깊은 복사해서 DummyRow에 저장한다.
						this->glyph->Add(0, letter->Clone());
						//2.6.7.4.6.3 글자를 지운다.
						currentRow->Remove(letterCount);
						letterCount--;
					}
					//2.6.7.4.7 다음 줄을 현재 줄에 Join시킨다.
					row->Join(currentRow);
					//2.6.7.4.8 다음 줄이 현재 줄에 Join되었기 때문에 다음줄의 주소를 노트에서 지운다.
					this->notepadForm->note->Remove(currentRowIndex + 1);
					//2.6.7.4.9 현재 글자의 위치를 다시 이동시켜준다.
			...
			{
				//2.6.8.1 이동하기 전의 줄의 위치와 이동한 후의 줄의 위치가 서로 같으면
				if (previousRowIndex == currentRowIndex)
				{
					//2.6.8.1.1 왼쪽 방향으로 단어단위로 이동한 뒤의 글자위치가 
				...
				//2.6.8.2 이동하기 전의 줄의 위치와 이동한 후의 줄의 위치가 서로 다르면
				else if (previousRowIndex > currentRowIndex)
				{
					//2.6.8.2.1 이동하기 전의 줄을 구한다.
					nextRow = this->notepadForm->note->GetAt(previousRowIndex);
					//2.6.8.2.2 이동하기 전의 글자위치까지 반복한다.
					i = 0;
					while (i < previousLetterIndex)
					{
						//2.6.8.2.2.1 글자를 지운다.
					...
					//2.6.8.2.3 나중에 현재 줄에 Join시키기 위해 줄의 주소를 저장한다.
					row = nextRow;
					//2.6.8.2.4 이동하기 전의 줄이 이동한 후의 줄의 위치보다 큰 동안 반복한다.
					previousRowIndex--;
					while (previousRowIndex > currentRowIndex)
					{
						//2.6.8.4.4.1 이동하기 전의 줄을 구한다.
						nextRow = this->notepadForm->note->GetAt(previousRowIndex);
						i = nextRow->GetLength() - 1;
						//2.6.8.4.4.2 이동하기 전의 줄의 개수만큼 반복한다.
						while (i >= 0)
						{
							//2.6.8.4.4.2.1 글자를 지운다.
							nextRow->Remove(i);
							i--;
						}
						//2.6.8.4.4.3 글자를 다지웠기 때문에 줄을 지운다.
						this->notepadForm->note->Remove(previousRowIndex);
						//2.6.8.4.4.4 이전 줄로 이동한다.
						previousRowIndex--;
					}
					//2.6.8.2.5 현재 줄을 구한다.
					currentRow = this->notepadForm->current;
					//2.6.8.2.6 이전 줄을 다 지웠기 때문에 현재 줄의 글자들을 지운다.
					//현재 줄의 글자 개수보다 현재 글자의 위치가 크거나 같은 동안 반복한다.
					letterCount = currentRow->GetLength() - 1;
					while (letterCount >= currentLetterIndex)
					{
					...
					//2.6.8.2.7 다음 줄을 현재 줄에 Join시킨다.
					row->Join(currentRow);
					//2.6.8.2.8 다음 줄이 현재 줄에 Join되었기 때문에 다음줄의 주소를 노트에서 지운다.
					this->notepadForm->note->Remove(currentRowIndex + 1);
					//2.6.8.2.9 현재 글자의 위치를 다시 이동시켜준다.
					currentLetterIndex = currentRow->Move(currentLetterIndex);
				}
			}
			//2.6.7.2 Command에 변경 사항이 있음을 표시한다.
			this->isDirty = true;
		}
		//2.7 변경사항이 있으면
...
//시작하는 줄의 위치구하기
Long CtrlBackSpaceKeyActionCommand::GetStartYPos()
{
	return this->startYPos;
}
//시작하는 글자의 위치구하기
Long CtrlBackSpaceKeyActionCommand::GetStartXPos()
{
	return this->startXPos;
}
//선택이 끝나는 줄의 위치 구하기
Long CtrlBackSpaceKeyActionCommand::GetSelectedEndYPos()
{
	return this->selectedEndYPos;
}
//선택이 끝나는 글자 위치 구하기
Long CtrlBackSpaceKeyActionCommand::GetSelectedEndXPos()
{
...
//선택영역이 지워졌는지 확인 여부
bool CtrlBackSpaceKeyActionCommand::IsSelectedTextsRemoved()
{
	return this->isSelectedTextsRemoved;
}

Glyph* CtrlBackSpaceKeyActionCommand::GetGlyph()
{
	return this->glyph;
}
