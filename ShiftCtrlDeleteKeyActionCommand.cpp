...
void ShiftCtrlDeleteKeyActionCommand::Execute()
{
	...
	if (this->notepadForm->isSelecting == false && this->isSelectedTextsRemoved == false)
	{
		...
		else
		{
			...
			if (this->isRedone == false)
			{
				//2.9.1.1 DummyRow를 생성한다.
				this->glyph = new DummyRow();
				//2.9.1.2 Command에 변경 사항이 있음을 표시한다.
				this->isDirty = true;
				//현재 줄의 글자들을 지운다.
				//2.9.1.3 현재글자위치가 마지막 글자위치보다 작은동안 반복한다.
				while (currentLetterPos < lastLetterPos)
				{
					//2.9.1.3.1 글자를 지우기 전에 글자를 구한다.
					letter = currentRow->GetAt(currentLetterPos);
					//2.9.1.3.2 글자를 깊은 복사해서 DummyRow에 저장한다.
					...
					dynamic_cast<DummyRow*>(nextRow))
				{
					...
					//2.9.1.4.2 다음 줄의 글자위치를 초기화한다.
					nextRowLetterPos = 0;
					//2.9.1.4.3 다음 줄의 글자위치가 마지막 글자위치보다 작은동안 반복한다.
					while (nextRowLetterPos < lastLetterPos)
					{
						//2.9.1.4.3.1.1 글자를 지우기 전에 글자를 구한다.
						letter = nextRow->GetAt(nextRowLetterPos);
						//2.9.1.4.3.1.2 글자를 깊은 복사해서 DummyRow에 저장한다.
						this->glyph->Add(letter->Clone());
						//2.9.1.4.3.1.3 글자를 지운다.
						nextRow->Remove(nextRowLetterPos);
						lastLetterPos--;
					}
					//2.9.1.4.4 글자가 다지워졌기 때문에 Note에서 다음 줄의 주소를 지운다.
					this->notepadForm->note->Remove(nextRowPos);
					//2.9.1.4.5 다음 줄의 주소를 저장한다.
			...
			{
				//현재 줄의 글자들을 지운다.
				//2.9.2.1 현재글자위치가 마지막 글자위치보다 작은동안 반복한다.
				while (currentLetterPos < lastLetterPos)
				{
					//2.9.2.1.1 글자를 지운다.
					currentRow->Remove(currentLetterPos);
				...
				//2.9.2.2 다음 줄이 가짜 줄인동안 반복한다. 
				while (dynamic_cast<DummyRow*>(nextRow))
				{
					//2.9.2.2.1 다음 줄의 마지막 글자위치를 구한다.
					lastLetterPos = nextRow->GetLength();
					//2.9.2.2.2 다음 줄의 글자위치를 초기화한다.
					nextRowLetterPos = 0;
					//2.9.2.2.3 다음 줄의 글자위치가 마지막 글자위치보다 작은동안 반복한다.
					while (nextRowLetterPos < lastLetterPos)
					{
						//2.9.2.2.3.1 글자를 지운다.
						nextRow->Remove(nextRowLetterPos);
					...
					//2.9.2.2.4 글자가 다지워졌기 때문에 Note에서 다음 줄의 주소를 지운다.
					this->notepadForm->note->Remove(nextRowPos);
					//2.9.2.2.5 다음 줄의 주소를 저장한다.
					nextRow = this->notepadForm->note->GetAt(nextRowPos);
				}
			}
...
//시작하는 줄의 위치구하기
Long ShiftCtrlDeleteKeyActionCommand::GetStartYPos()
{
	return this->startYPos;
}
//시작하는 글자의 위치구하기
Long ShiftCtrlDeleteKeyActionCommand::GetStartXPos()
{
	return this->startXPos;
}
//선택이 끝나는 줄의 위치 구하기
Long ShiftCtrlDeleteKeyActionCommand::GetSelectedEndYPos()
{
	return this->selectedEndYPos;
}
//선택이 끝나는 글자 위치 구하기
Long ShiftCtrlDeleteKeyActionCommand::GetSelectedEndXPos()
{
...
Glyph* ShiftCtrlDeleteKeyActionCommand::GetGlyph()
{
	return this->glyph;
}
