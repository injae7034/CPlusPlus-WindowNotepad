...
void PageUpKeyAction::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	//13. 수직스크롤을 이전 페이지로 이동시키고 이동된 수직스크롤의 현재 위치를 반환받는다.
	...
	{
		//14.1 수직스크롤바의 수직스크롤을 한 줄 이전으로 이동시킨다.
		this->notepadForm->SetScrollPos(SB_VERT,
			this->notepadForm->scrollController->scroll[1]->GetCurrentPos());
		...
		if (previousCaretIndex != 0 && this->notepadForm->current->GetLength() != 0)
		{
			...
			//14.8.2 캐럿의 현재 가로 위치를 처음으로 이동시킨다.(현재 캐럿의 가로 위치가 어딘지 모르기때문에)
			...
			//14.8.3 첫번째 글자를 읽기 위해 캐럿을 한칸이동시킨다.
