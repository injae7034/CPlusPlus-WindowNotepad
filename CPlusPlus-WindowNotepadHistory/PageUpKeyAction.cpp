...
void PageUpKeyAction::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	//4. 수직스크롤을 이전 페이지로 이동시키고 이동된 수직스크롤의 현재 위치를 반환받는다.
	...
	{
		//5.1 수직스크롤바의 수직스크롤을 한 페이지 이전으로 이동시킨다.
		this->notepadForm->SetScrollPos(SB_VERT,
			this->notepadForm->scrollController->scroll[1]->GetCurrentPos());
		//5.2 Note에서 캐럿이 실제 이동할 거리를 구한다.
		...
		if (previousCaretIndex != 0 && this->notepadForm->current->GetLength() != 0)
		{
			...
			//5.8.2 캐럿의 현재 가로 위치를 처음으로 이동시킨다.(현재 캐럿의 가로 위치가 어딘지 모르기때문에)
			...
			//5.8.3 첫번째 글자를 읽기 위해 캐럿을 한칸이동시킨다.
			Long i = this->notepadForm->current->Next();
			//5.8.4 현재 줄의 텍스트의 폭을 구한다.
