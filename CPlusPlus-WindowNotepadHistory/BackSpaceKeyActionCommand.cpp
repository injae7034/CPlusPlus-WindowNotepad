...
void BackSpaceKeyActionCommand::Execute()
{
	...
	if (this->notepadForm->isSelecting == false)
	{
		...
		{
			//3.2.1 현재 글자를 지우기 전에 지울 글자를 구한다.
			Glyph* letter = this->notepadForm->current->GetAt(currentLetterPos - 1);
			//3.2.2 기존에 글자가 있으면 글자를 할당해제한다.
			if (this->glyph != 0)
			{
				delete this->glyph;
			}
			//3.2.2 현재 글자를 깊은 복사해서 저장한다.
			this->glyph = letter->Clone();
			//3.2.3 현재 글자를 지운다.
			this->notepadForm->current->Remove(currentLetterPos - 1);
			//3.2.4 자동 줄 바꿈 메뉴가 체크되어 있으면
			if (this->notepadForm->isRowAutoChanging == true)
			{
				//3.2.4.1 OnSize로 메세지가 가지 않기 때문에 OnSize로 가는 메세지를 보내서
...
void BackSpaceKeyActionCommand::Unexecute()
{
	...
	else
	{
		...
		//4.3 기존에 저장된 줄이 있으면 할당해제한다.
		if (this->glyph != 0)
		{
			delete this->glyph;
		}
		//4.4. 현재 줄에서 현재 글자 다음 위치에 있는 글자들을 떼어내 새로운 줄을 만들어 저장한다.
		this->glyph = this->notepadForm->current->Split(currentLetterPos);
		//4.5 현재 줄의 위치가 노트의 줄의 개수-1 과 같고(현재 줄의 위치가 마지막 줄이면)
		if (currentRowPos == this->notepadForm->note->GetLength() - 1)
		{
			//4.5.1 새로운 줄을 마지막 줄 다음에 추가한다.
			currentRowPos = this->notepadForm->note->Add(this->glyph->Clone());
		}
		//4.6 그게 아니면
		else
		{
			//4.6.1 새로운 줄을 현재 줄의 다음 위치에 끼워 넣는다.
			currentRowPos = this->notepadForm->note->
				Add(currentRowPos + 1, this->glyph->Clone());
		}
		//4.7 현재 줄을 새로 저장한다.
		this->notepadForm->current = this->notepadForm->note->GetAt(currentRowPos);
		//4.8 현재 줄의 글자 위치를 처음으로 이동시킨다.
		this->notepadForm->current->First();
		//4.9 자동 줄 바꿈이 진행중이면
		if (this->notepadForm->isRowAutoChanging == true)
		{
			//4.9.1 OnSize로 메세지가 가지 않기 때문에 OnSize로 가는 메세지를 보내서
			//OnSize에서 부분자동개행을 하도록 한다. 
			this->notepadForm->SendMessage(WM_SIZE);
		}
	}
