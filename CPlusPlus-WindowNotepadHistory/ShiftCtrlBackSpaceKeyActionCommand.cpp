...
void ShiftCtrlBackSpaceKeyActionCommand::Execute()
{
	...
	if (this->notepadForm->isSelecting == false && this->isSelectedTextsRemoved == false)
	{
		...
		else
		{
			...
			if (this->isRedone == false)
			{
				//2.6.1.1 DummyRow를 생성한다.
				this->glyph = new DummyRow();
				//2.6.1.2 Command에 변경 사항이 있음을 표시한다.
				this->isDirty = true;
			}
			//2.6.2 현재 줄이 가짜줄이면
			if (dynamic_cast<DummyRow*>(currentRow))
			{
				...
				if (this->isRedone == false)
				{
					//2.6.2.1.1 현재 글자위치가 0보다 작은동안 반복한다.
					while (currentLetterPos > 0)
					{
						//2.6.2.1.1.1 글자를 지우기 전에 글자를 구한다.
						letter = currentRow->GetAt(currentLetterPos - 1);
						//2.6.2.1.1.2 글자를 깊은 복사해서 DummyRow에 저장한다.
				...
				{
					//2.6.2.2.1 현재 글자위치가 0보다 작은동안 반복한다.
					while (currentLetterPos > 0)
					{
						//2.6.2.2.1.1 글자를 지운다.
						currentRow->Remove(currentLetterPos - 1);
			...
			if (this->isRedone == false)
			{
				//2.6.3.1 가짜 줄인동안 반복한다. 
				while (dynamic_cast<DummyRow*>(currentRow))
				{
					//2.6.3.1.1 현재 글자위치가 0보다 작은동안 반복한다.
					while (currentLetterPos > 0)
					{
						//2.6.3.1.1.1 글자를 지우기 전에 글자를 구한다.
						letter = currentRow->GetAt(currentLetterPos - 1);
						//2.6.3.1.1.2 글자를 깊은 복사해서 DummyRow에 저장한다.
						this->glyph->Add(0, letter->Clone());
						//2.6.3.1.1.3 글자를 지운다.
						currentRow->Remove(currentLetterPos - 1);
						currentLetterPos--;
					}
					//2.6.3.1.2 글자가 다지워졌기 때문에 Note에서 현재 줄의 주소를 지운다.
					this->notepadForm->note->Remove(currentRowPos);
					//2.6.3.1.3 이전 줄의 마지막 글자위치로 이동한다.
					currentRowPos = this->notepadForm->note->Move(currentRowPos - 1);
					this->notepadForm->current = this->notepadForm->note->GetAt(currentRowPos);
					currentLetterPos = this->notepadForm->current->Last();
					currentRow = this->notepadForm->current;
				}
				//2.6.3.2 진짜 줄의 글자를 다 지우는 동안 반복한다.
				while (currentLetterPos > 0)
				{
					//2.6.3.2.1 글자를 지우기 전에 글자를 구한다.
					letter = currentRow->GetAt(currentLetterPos - 1);
					//2.6.3.2.2 글자를 깊은 복사해서 DummyRow에 저장한다.
					this->glyph->Add(0, letter->Clone());
					//2.6.3.2.3 글자를 지운다.
					currentRow->Remove(currentLetterPos - 1);
					currentLetterPos--;
