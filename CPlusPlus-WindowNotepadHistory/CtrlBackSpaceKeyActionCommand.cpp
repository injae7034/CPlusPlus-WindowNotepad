...
void CtrlBackSpaceKeyActionCommand::Execute()
{
	...
	if (this->notepadForm->isSelecting == false && this->isSelectedTextsRemoved == false)
	{
		...
		else
		{
			...
			{
				//4.4.3.1 노트에서 단어단위로 왼쪽으로 이동한다.
				currentLetterIndex = this->notepadForm->note->PreviousWord();
				//4.4.3.2 이동한 후에 줄의 위치를 구한다.
				currentRowIndex = this->notepadForm->note->GetCurrent();
				//4.4.3.3 메모장의 현재 줄을 이동한 후의 줄로 변경한다.
				this->notepadForm->current = this->notepadForm->note->GetAt(currentRowIndex);
			}
			//4.4.4 자동개행이 진행 중이면
			else
			{
				//4.4.4.1 노트에서 자동개행 진행 중인 상태에서 단어단위로 왼쪽으로 이동한다.
				this->notepadForm->note->
					PreviousWordOnRowAutoChange(currentRowIndex, currentLetterIndex);
				//4.4.4.2 이동한 후에 줄의 위치를 구한다.
				currentRowIndex = this->notepadForm->note->GetCurrent();
				//4.4.4.3 메모장의 현재 줄을 이동한 후의 줄로 변경한다.
				this->notepadForm->current = this->notepadForm->note->GetAt(currentRowIndex);
			}
			//4.4.5 왼쪽 방향으로 단어단위로 이동한 다음에 현재 줄의 위치를 구한다.
			currentRowIndex = this->notepadForm->note->GetCurrent();
			this->notepadForm->current = this->notepadForm->note->GetAt(currentRowIndex);
...
//선택이 시작되는 글자의 위치구하기
Long CtrlBackSpaceKeyActionCommand::GetSelectedStartXPos()
{
	return this->selectedStartXPos;
}

//선택이 시작되는 줄의 위치구하기
Long CtrlBackSpaceKeyActionCommand::GetSelectedStartYPos()
{
