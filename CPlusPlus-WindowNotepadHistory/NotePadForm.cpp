...
void NotepadForm::OnPaint()
{
	...
	//8. RowTextOutVisitor를 생성한다.
	RowTextOutVisitor* rowTextOutVisitor = new RowTextOutVisitor(this);
	//9. LetterTextOutVisitor를 생성한다.
	LetterTextOutVisitor* letterTextOutVisitor = new LetterTextOutVisitor(this);
	//10. 선택된 범위를 구한다.
	...
	//11. rowIndex가 선택영역 시작줄보다 작은동안 반복한다.
	Glyph* row = 0;
	Long rowIndex = 0;
	while (rowIndex < startingRowPos)
	{
		...
		//11.2 row에 rowTextOutVisitor를 Accept한다.
		row->Accept(rowTextOutVisitor);
	}
