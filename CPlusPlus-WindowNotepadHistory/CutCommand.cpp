...
{
#if 0
	//잘라내기 Command에 들어왔다는 말은 최소 한글자이상은 잘라낼게 있어서 들어왔다는 뜻임
	//잘라낼게 없었으면 애초에 여기에 들어올 수 없음 그러므로 잘라내기의 Execute에서는
	//노트를 생성하고 줄을 한 개 생성해서 새로 생성한 노트에 추가까지 하는게 디폴트이다.
	//1. 잘라낸 텍스트를 담을 새노트를 생성한다.
	Glyph* copyNote = new Note();
	//2. 줄을 생성한다.(잘라낸 내용이 한 글자이든 수천줄이든 일단 무조건 줄은 하나 생성해야함.)
	//그리고 자르기를 시작한 줄이 가짜줄이든 진짜줄이든 상관없이 처음 복사한 줄은 무조건 Row로 생성함. 
	...
	if (selectedStartRowPos == selectedEndRowPos)
	{
		...
		//7.1.4 시작하는 글자위치부터 끝나는 글자까지 복사하고 지운다.
		i = startLetterIndex;
		while (i < endLetterIndex)
		{
			...
			//7.1.4.4 글자를 내부클립보드에 복사했으면 해당 글자를 지운다.
			startRow->Remove(i);
			//7.1.4.5 글자를 지웠기 때문에 끝나는 글자위치를 한 칸 앞으로 당긴다.
			endLetterIndex--;
		}
	}
	...
	else
	{
		...
		//8.3 시작하는 줄을 구한다.
		startRow = this->notepadForm->note->GetAt(startRowIndex);
		//8.4 시작하는 글자위치부터 시작하는 줄의 마지막 글자까지 복사하고 글자를 지운다.
		//<주의> 시작하는 줄은 설령 글자가 다지워졌다고 해도 절대로 그 줄을 노트에서 지우면 안된다!!!
		i = startLetterIndex;
		while (i < startRow->GetLength())
		{
			...
			copyRow->Add(letter->Clone());
			//8.4.4 글자를 내부클립보드에 복사했으면 해당 글자를 지운다.
			startRow->Remove(i);
		}
		...
		while (nextRowIndex < endRowIndex)
		{
			...
			if (!dynamic_cast<DummyRow*>(row))
			{
				//8.7.2.1 진짜 줄을 생성한다.
				realRow = new Row();
				//8.7.2.2 진짜 줄을 새로 만든 노트에 추가한다.
				rowIndexOfCopyNote = copyNote->Add(realRow);
			}
			//8.7.3 메모장의 진짜 줄의 처음부터 마지막 글자까지 반복한다.
			letterIndex = 0;
			while (letterIndex < row->GetLength())
			{
				//8.7.3.1 메모장의 진짜 줄에서 글자를 구한다.
				letter = row->GetAt(letterIndex);
				//8.7.3.2 글자가 선택이 되어있는데 클립보드에 생성된 새로운 노트에 옮길 때는
				//선택이 안된 상태로 복사해야함. 그래야 나중에 붙여넣기할때 
				//붙여넣는 내용들이 선택이 안된 상태로 붙여넣어짐!
				letter->Select(false);
				//8.7.3.3 새로 만든 노트의 새로 만든 진짜 줄에 글자를 추가한다.
				realRow->Add(letter->Clone());
				//8.7.3.4 글자를 내부클립보드에 복사했으면 해당 글자를 지운다.
				row->Remove(letterIndex);
			}
			//8.7.4 줄의 글자가 다지워졌기 때문에 줄을 지운다.
			this->notepadForm->note->Remove(nextRowIndex);
			//8.7.5 줄이 지워졌기 때문에 마지막 줄을 한 줄 앞당긴다
			endRowIndex--;
		}
		//8.8 메모장에서 끝나는 줄을 구한다.
		endRow = this->notepadForm->note->GetAt(nextRowIndex);
		//8.9 메모장에서 끝나는 줄이 진짜줄이면
		if (!dynamic_cast<DummyRow*>(endRow))
		{
			//8.9.1 진짜 줄을 생성한다.
			realRow = new Row();
			//8.9.2 진짜 줄을 새로 만든 노트에 추가한다.
			rowIndexOfCopyNote = copyNote->Add(realRow);
		}
		//새로운 노트의 진짜 줄에 메모장의 줄의 글자를 복사하고 지운다.
		//8.10 글자 위치를 원위치시킨다.
		letterIndex = 0;
		//8.11 메모장의 끝나는 줄에서 마지막 글자까지 반복한다.
		while (letterIndex < endLetterIndex)
		{
			//8.11.1 메모장의 끝나는 줄에서 글자를 구한다.
			letter = endRow->GetAt(letterIndex);
			//8.11.2 글자가 선택이 되어있는데 클립보드에 생성된 새로운 노트에 옮길 때는 선택이 안된
			//상태로 복사해야함. 그래야 나중에 붙여넣기할때 붙여넣는 내용들이 선택이 안된 상태로 붙여넣어짐!
			letter->Select(false);
			//8.11.3 새로운 노트의 진짜 줄에 글자를 순차적으로 추가한다.
			realRow->Add(letter->Clone());
			//8.11.4 글자를 내부클립보드에 복사했으면 해당 글자를 지운다.
			endRow->Remove(letterIndex);
			//8.11.5 글자를 지웠기 때문에 끝나는 글자위치를 한 칸 앞으로 당긴다.
			endLetterIndex--;
		}
		//8.12 메모장의 끝나는 줄을 메모장의 시작하는 줄에 Join시킨다.
		endRow->Join(startRow);
		//8.13. 메모장에서 끝나는 줄을 지운다.
		this->notepadForm->note->Remove(startRowIndex);
	}
