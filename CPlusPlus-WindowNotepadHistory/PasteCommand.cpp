	...
	if (this->glyph != 0)
	{
		...
		if (this->notepadForm->isSelecting == true || this->isSelectedTextsRemoved == true)
		{
			...
			if (this->isRedone == false)
			{
				...
				//5.1.3.2 Command에 변경 사항이 있음을 표시한다.
				this->isDirty = true;
			}
			//5.1.4 다시 실행이면
			else
			...
			//5.1.5 연산이 끝났기 때문에 현재 줄의 위치를 다시 조정해준다.(note의연산안에서 현재 줄의 위치와 글자 위치는
		...
		//5.2 Command에 변경 사항이 있으면
		if (this->isDirty == true)
		{
			//5.2.1 메모장 제목에 *를 추가한다.
			string name = this->notepadForm->fileName;
			name.insert(0, "*");
			name += " - 메모장";
			this->notepadForm->SetWindowText(CString(name.c_str()));
			//5.2.2 메모장에 변경사항이 있음을 저장한다.
			this->notepadForm->isDirty = true;
		}
		//5.3 붙여넣기 전에 붙여넣기가 시작되는 줄의 위치를 저장한다.
		this->startPasteRowIndex = currentRowPos;
		//5.4 붙여넣기 전에 붙여넣기가 시작되는 글자의 위치를 저장한다.
		this->startPasteLetterIndex = currentLetterPos;
		//5.5 현재 위치에 command가 가지고 있는 content를 삽입한다.
		...
		//5.8 글자를 추가한 후에 현재 줄의 위치와 글자위치를 다시 저장한다.
