...
void PasteCommand::Execute()
{
	...
	if (this->glyph != 0)
	{
		//6.1 붙여넣기 전에 붙여넣기가 시작되는 줄의 위치를 저장한다.
		this->startPasteRowIndex = currentRowPos;
		//6.2 붙여넣기 전에 붙여넣기가 시작되는 글자의 위치를 저장한다.
		this->startPasteLetterIndex = currentLetterPos;
		//6.3 현재 위치에 command가 가지고 있는 content를 삽입한다.
		...
		//6.5 자동개행이 진행중이면 붙여넣은 줄들을 자동개행시켜준다.
		if (this->notepadForm->isRowAutoChanging == true)
		{
			//6.5.1 자동개행클래스를 생성한다.
			RowAutoChange rowAutoChange(this->notepadForm);
		...
		//6.7 메모장에 변경사항이 있음을 저장한다.
		this->notepadForm->isDirty = true;
		//6.8 글자를 지운 후에 현재 줄의 위치와 글자위치를 다시 저장한다.
		this->rowIndex = this->notepadForm->note->GetCurrent();
		this->notepadForm->current = this->notepadForm->note->GetAt(this->rowIndex);
		this->letterIndex = this->notepadForm->current->GetCurrent();
		//6.9 자동개행이 진행중이면(command의 줄과 글자 위치는 항상 진짜 줄과 글자 위치를 저장해야함)
		if (this->notepadForm->isRowAutoChanging == true)
		{
			...
			//6.9.1 변경된 화면 크기에 맞는 줄과 캐럿의 위치를 구한다.
			rowAutoChange.GetOriginPos(changedLetterPos, changedRowPos, &originLetterPos,
				&originRowPos);
			//6.9.2 command에 글자를 입력한 후에 현재 줄의 위치와 글자위치를 다시 저장한다.
			this->rowIndex = originRowPos;
			this->letterIndex = originLetterPos;
		}
		//6.10 command에 변경사항(붙여넣은 내용)이 있음을 표시한다.
		this->isDirty = true;
	}
}
...
void PasteCommand::Unexecute()
{
	...
	if (this->isSelectedTextsRemoved == true)
	{
		...
		if (this->notepadForm->isRowAutoChanging == true)
		{
			//10.7.1 자동개행클래스를 생성한다.
			RowAutoChange rowAutoChange(this->notepadForm);
			//10.7.2 부분자동개행을 한다.
			Long endPastedRowPos = rowAutoChange.DoPartRows(currentRowPos, rowIndex);
			//10.7.3 붙여넣기가 끝나는 줄로 이동시킨다.
			//붙여넣기가 끝나는 줄은 OnSize에서 부분자동개행을 해서 처리되기 때문에 캐럿의 위치만 조정해주면 됨!
			currentRowPos = this->notepadForm->note->Move(endPastedRowPos);
			this->notepadForm->current = this->notepadForm->note->GetAt(currentRowPos);
			currentLetterPos = this->notepadForm->current->GetCurrent();
