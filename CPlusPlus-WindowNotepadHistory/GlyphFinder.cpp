    ...
    Long* findingEndRowIndex, Long* findingEndLetterIndex)
{
    ...
    Long i = 0;//줄의 content에서 글자를 읽을 배열요소 및 반복제변수
    Long j = keywordLength;//찾을 문자열의 content에서 글자를 읽을 배열요소 및 반복제어변수
    ...
    while (currentRowIndex >= 0 && isMatched == false)
    {
        ...
        if (dynamic_cast<DummyRow*>(row))
        {
            ...
            while (dynamic_cast<DummyRow*>(previousRow))
            {
                ...
                //8.3.3.2 이전 줄의 content 길이를 구한다.
                previousRowContentLength = previousRowContent.length();
                //8.3.3.3 메모장의 현재 줄에서 현재글자까지의 content길이에 이전줄의
                //content길이를 더해준다.
                currentLettersLength += previousRowContentLength;
                //8.3.3.4 메모장의 현재줄에서 현재 글자위치에 이전 줄의 글자개수를 더해준다.
                currentLetterIndex += previousRow->GetLength();
                //8.3.3.5 이전 줄의 content에 현재 줄의 cotent를 더해준다.(누적)
                previousRowContent += rowContent;
                //8.3.3.6 더해준 값을 옮겨 저장한다.(반복 구조에서 제대로 누적시키기 위해서)
        ...
        //8.4 자동개행이 안된 상태의 전체 줄의 길이를 구한다.
        rowContentLength = rowContent.length();
        //8.5 줄에서 읽을 글자 위치를 초기화시킨다.
        i = currentLettersLength;
        ...
        while (i >= 0 && isMatched == false)
        {
           ...
           //일치하는 문자열의 길이와 찾을 문자열의 길이가 일치하면
            if (matchedLength == keywordLength)
            {
                ...
                currentRowContent = currentRow->GetContent();
                //8.6.8.4 현재 줄의 content의 length를 구한다.
                currentRowContentLength = currentRowContent.length();
                //8.6.8.5 i가 전체줄의 contentLength에서 현재 줄의 contentLength를 뺀값보다 큰동안
                //반복한다.
                minLength = rowContentLength - currentRowContentLength;
                while (i > minLength)
                {
                    //8.6.8.5.1 현재 줄의 위치를 감소시킨다.
                    //자동개행이 취소된 상태에서 해당 문자열을 찾았는데 최종적으로 읽은 글자가
                    //현재 개별적인 줄의 길이보다 길기 때문에 이전 줄로 넘어가서 다시 비교해야한다.
                    currentRowIndex--;
                    //8.6.8.5.2 현재 글자위치도 자동개행이 취소된 상태의 글자위치기 때문에
                    //현재 글자위치가 현재 줄의 개수보다 크기 때문에 그만큼 빼서 자동개행이 된 상태로 조정한다
                    //앞에서 currentLetterIndex++로 인해 1이 증가된 상태이기 때문에 +1을 따로 안해줘도됨!
                    currentLetterIndex -= currentRow->GetLength();
                    //8.6.8.5.3 현재 줄의 이전 줄을 구한다.
                    previousRow = this->note->GetAt(currentRowIndex);
                    //8.6.8.5.4 이전 줄의 content를 구한다.
                    previousRowContent = previousRow->GetContent();
                    //8.6.8.5.5 이전 줄의 content의 길이를 구한다.
                    previousRowContentLength = previousRowContent.length();
                    //8.6.8.5.6 전체 줄의 contentLength에 이전 줄의 길이를 빼준다(누적)
                    rowContentLength -= previousRowContentLength;
                }
            ...
            if (isMatched == false)
            {
                //8.7.1 현재 줄의 위치를 이전 줄의 다음 위치로 다시 재조정한다.
