...
void CtrlDeleteKeyAction::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (this->notepadForm->isSelecting == false)
	{
		...
		{
			//3.4.1 오른쪽 방향으로 단어단위로 이동한 뒤의 글자위치를 구한다.
			Long letterPosAfterMoving = this->notepadForm->current->NextWord();
			//3.4.2 오른쪽 방향으로 단어단위로 이동한 뒤의 글자위치가 현재글자위치보다 큰동안 반복한다.
			...
			//3.4.3 메모장 제목에 *를 추가한다.
			string name = this->notepadForm->fileName;
			name.insert(0, "*");
			name += " - 메모장";
			this->notepadForm->SetWindowText(CString(name.c_str()));
			//3.4.4 메모장에 변경사항이 있음을 저장한다.
			this->notepadForm->isDirty = true;
			//3.4.5 자동 줄 바꿈 메뉴가 체크되었는지 확인한다.
			UINT state = this->notepadForm->GetMenu()->
				GetMenuState(IDM_ROW_AUTOCHANGE, MF_BYCOMMAND);
			//3.4.6 자동 줄 바꿈 메뉴가 체크되어 있으면
			if (state == MF_CHECKED)
			{
				//3.4.6.1 OnSize로 메세지가 가지 않기 때문에 OnSize로 가는 메세지를 보내서
