	...
	:GlyphVisitor(notepadForm, dc, glyphXPos, glyphYPos)
{
	this->tempDC = 0;
	this->maxRowWidth = 0;
	this->isRowWidthOverRectWidth = false;
}
	...
	:GlyphVisitor(source)
{
	...
	this->maxRowWidth = source.maxRowWidth;
	this->isRowWidthOverRectWidth = source.isRowWidthOverRectWidth;
}
...
#if 0
	if (this->isRowWidthOverRectWidth == true)
	{
		//2. 프린트 가능한 영역을 구한다. 
		rect = CRect(0, 0, this->maxRowWidth, cdc->GetDeviceCaps(VERTRES) + text.tmHeight);

		//3. 글자를 출력할 비트맵을 생성한다.(프린트 화면이 메모장 화면보다 더크기 때문에 메모장 화면에서
		//프린트 화면을 제대로 보여주기 위해서는 축소를 시켜줘야하는데 축소시키기 위해서는 비트맵을 생성해야한다.)
		//dcText.CreateCompatibleDC(this->dc);
		hbmp = ::CreateCompatibleBitmap(dcText, rect.Width(), rect.Height());
		oldBMP = (HBITMAP)dcText.SelectObject(hbmp);
		//4. 텍스트가 출력될 부분의 배경을 흰색으로 칠한다.
		...
		dcText.FillRect(&rect, CBrush::FromHandle((HBRUSH)GetStockObject(WHITE_BRUSH)));
		//8. 비트맵 축소 크기를 구한다.
		downsizedWidth = rect.Width() / 10;
		downsizedHeight = rect.Height() / 10;
		//9. 페이지 설정 정보가 있으면
		glyphXPos = 0;
		if (this->notepadForm->pageSetUpInformation != NULL)
		{
...
void PreviewVisitor::VisitRow(Glyph* row)
{
	...
	//2. 프린트 가능한 영역을 구한다. 
	CRect rect(0, 0, cdc->GetDeviceCaps(HORZRES), cdc->GetDeviceCaps(VERTRES));
	Long rowWidth = this->glyphXPos + letterWidth;
	if (rowWidth >= rect.Width())
	{
		this->isRowWidthOverRectWidth = true;
